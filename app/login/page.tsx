...
function SuccessCard({
  ...
  const handleGoogle = async () => {
    setSigning(true);
    const callbackUrl = `/account?sig=${encodeURIComponent(txSignature ?? "")}&wallet=${encodeURIComponent(walletAddress ?? "")}`;
    ...
    <div className="glass-card rounded-3xl p-7 md:p-9">
      ...
      <div className="text-center mb-8">
        <div className="relative inline-flex items-center justify-center mb-5">
          ...
          <span className="absolute -top-1 -right-1 w-6 h-6 rounded-full bg-[#0A0A0A] flex items-center justify-center text-xs">⛓</span>
        </div>
        <h2 className="font-heading font-bold text-3xl mb-2">Payment Confirmed</h2>
        <p className="text-[#10B981] text-sm font-mono">Verified on Solana Devnet</p>
      </div>

      {/* ── On-chain proof strip ── */}
      {txSignature && (
        <div className="bg-white/[0.025] border border-white/[0.07] rounded-2xl px-4 py-3 mb-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span className="w-1.5 h-1.5 rounded-full bg-[#10B981]" />
              <span className="text-white/30 text-[10px] font-mono uppercase tracking-widest">On-chain proof</span>
            </div>
            <a
              href={`${SOLSCAN_BASE}${txSignature}?cluster=devnet`}
              target="_blank"
              rel="noopener noreferrer"
              className="inline-flex items-center gap-1.5 text-[11px] font-semibold text-[#3B82F6] hover:text-blue-400 transition-colors"
            >
      ...
      <div
        ...
        style={{ background: "linear-gradient(135deg, rgba(153,69,255,0.1), rgba(59,130,246,0.07))", border: "1px solid rgba(153,69,255,0.25)" }}
      >
        <div className="flex items-start gap-3 mb-4">
          <div
            className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0"
            style={{ background: "rgba(153,69,255,0.15)", border: "1px solid rgba(153,69,255,0.25)" }}
          >
            <svg className="w-4.5 h-4.5 text-[#9945FF]" width="18" height="18" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
            </svg>
          </div>
          <div>
            <p className="text-white font-heading font-semibold text-sm leading-snug mb-1">
              Save your purchase to your account
            </p>
            <p className="text-white/40 text-xs font-body leading-relaxed">
              Your payment is confirmed on-chain. Continue with Google to save your purchase and view your full payment history. We will also send your course details and onboarding instructions after registration.
            </p>
          </div>
        </div>
        ...
        <button
          ...
          disabled={signing}
          className="w-full flex items-center justify-center gap-3 bg-white text-[#1a1a1a] font-heading font-bold text-sm px-4 py-3.5 rounded-xl transition-all hover:scale-[1.01] hover:bg-white/90 disabled:opacity-60 disabled:cursor-not-allowed"
        >
          {signing ? (
            <><div className="w-4 h-4 rounded-full border-2 border-[#1a1a1a]/20 border-t-[#1a1a1a] animate-spin" /> Redirecting to Google…</>
          ) : (
            <>
              {/* Google 'G' logo */}
      ...
      <div className="rounded-2xl overflow-hidden" style={{ border: "1px solid rgba(255,255,255,0.06)" }}>
        <div className="bg-white/[0.03] px-4 py-2.5 border-b border-white/[0.05]">
          <p className="text-white/30 text-[10px] font-mono uppercase tracking-widest">Your access includes</p>
        </div>
        <div className="divide-y divide-white/[0.04]">
          {[
            { icon: "🎓", label: "7 Hands-On Lectures" },
            { icon: "🚀", label: "Ship a Real Product" },
            { icon: "♾️", label: "Lifetime Access" },
          ].map(({ icon, label }) => (
            <div key={label} className="flex items-center gap-3 px-4 py-3">
              <span className="text-sm">{icon}</span>
              <span className="text-white/60 text-xs font-body">{label}</span>
              <svg className="w-3.5 h-3.5 text-[#10B981] ml-auto" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2.5} d="M5 13l4 4L19 7" />
              </svg>
            </div>
          ))}
